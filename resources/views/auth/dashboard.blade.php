                        ...
                        <div class="card-body">
                            ...
                            <div id="zipPreview" class="mt-3" style="display:none;">
                                <div class="file-item">
                                    ...
                                    <button class="btn btn-sm btn-outline-danger" onclick="clearZipSelection()">
                                        <i class="fas fa-times"></i>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="d-grid mt-3">
                                <button class="btn btn-primary-custom btn-lg" id="btnAnalyzeZip" onclick="analyzeZip()" disabled>
    ...
    html += `<div class="result-card mb-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h6 class="fw-bold mb-0">${data.filename}</h6>
                    <small class="text-muted">${getFileTypeBadge(data.detected_type)} &bull; ${formatSize(data.content_length || 0)} chars &bull; ${data.num_comparisons || 0} comparaisons</small>
                </div>
            </div>
            <div>
                ${data.plagiarism_detected
                    ? '<span class="badge bg-danger fs-6"><i class="fas fa-exclamation-triangle me-1"></i>Plagiat detecte</span>'
                    : '<span class="badge bg-success fs-6"><i class="fas fa-check me-1"></i>Aucun plagiat</span>'}
            </div>
        </div>
    </div>`;
    ...
    html += `<div class="result-card mb-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h6 class="fw-bold mb-0">${data.filename}</h6>
                    <small class="text-muted">${data.files_extracted ? data.files_extracted.length : 0} fichiers extraits &bull; ${data.files_analyzed || 0} analyses</small>
                </div>
            </div>
            <div>
                ${data.plagiarism_detected
            ...
            html += `<div class="result-card">
                <div class="d-flex justify-content-between align-items-start mb-1">
                    <div>
                        ...
                        <small class="text-muted">${getFileTypeBadge(r.file_type)} &bull; ${r.matches_count || 0} matchs ${r.best_match ? '&bull; Best: ' + r.best_match : ''}</small>
                    </div>
            ...
            html += `<div class="result-card">
                <div class="d-flex justify-content-between align-items-center">
                    <div style="font-size:14px;">
                        ...
                        <i class="fas fa-arrows-left-right mx-2 text-muted"></i>
                        <span class="fw-semibold">${c.file_b}</span>
                    </div>
    ...
    try {
        ...
        items.forEach(item => {
            html += `<tr>
                ...
                <td>${item.length ? item.length.toLocaleString() + ' chars' : '-'}</td>
                <td>${item.images || 0}</td>
